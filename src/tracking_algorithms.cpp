...
void PurePursuit::computeRearWheelPos(const sgtdv_msgs::CarPose::ConstPtr &carPose)
{
    ...
    rear_wheels_pos_ = pos - cv::Vec2f(cosf(carPose->yaw) * params_.rear_wheels_offset, sinf(carPose->yaw)  * params_.rear_wheels_offset);
#ifdef SGT_VISUALIZATION
    visualizePoint(rear_wheels_pos_, 2, "rear wheels" , cv::Vec3f(0.0, 0.0, 1.0));
    const auto frontWheelsPos = pos + cv::Vec2f(cosf(carPose->yaw) * params_.front_wheels_offset, sinf(carPose->yaw)  * params_.front_wheels_offset);
...
{
    const float lookahead_dist = params_.steering_k * carVel->speed;
...
cv::Vec2f PurePursuit::findTargetPoint(const sgtdv_msgs::Point2DArr::ConstPtr &trajectory) const
{
    ...
    */
    ...
    visualizePoint(cv::Vec2f(trajectory->points[center_line_idx].x, trajectory->points[center_line_idx].y), 1, "closest point", cv::Vec3f(1.0, 1.0, 0.0));
#endif /* SGT_VISUALIZATION */
...
float PurePursuit::computeSteeringCommand(const PathTrackingMsg &msg, const cv::Vec2f &targetPoint)
{
    ...
    const double alpha = std::atan2((targetPoint[1] - msg.car_pose->position.y),(targetPoint[0] - msg.car_pose->position.x)) - theta;
