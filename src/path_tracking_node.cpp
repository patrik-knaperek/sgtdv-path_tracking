...
int main (int argc, char** argv)
{
    ...
    ros::Publisher steering_pose_publisher = handle.advertise<geometry_msgs::PoseStamped>("pathtracking/visualize/steering", 1);
    ...
    ros::Subscriber trajectory_sub = handle.subscribe("pathplanning_trajectory", 10, &PathTrackingSynch::doPlannedTrajectory, &synch_obj);
    ...
    ros::Subscriber velocity_sub = handle.subscribe("velocity_estimate", 10, &PathTrackingSynch::doVelocityEstimate, &synch_obj);
    ros::ServiceServer stop_sub = handle.advertiseService("path_tracking/stop", &PathTrackingSynch::stopCallback, &synch_obj);
    ros::ServiceServer start_sub = handle.advertiseService("path_tracking/start", &PathTrackingSynch::startCallback, &synch_obj);
